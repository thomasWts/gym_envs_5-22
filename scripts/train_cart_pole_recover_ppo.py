...
MODEL_PATH = ROOT / "checkpoints" / "ppo_cart_pole_recover_torch.pt"
DEFAULT_INIT_PATH = ROOT / "checkpoints" / "ppo_cart_pole_balance_torch.pt"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train CartPoleRecoverEnv with PPO.",
    )
    parser.add_argument(
        "--init-from",
        type=Path,
        ...
        help=(
            "用已有 checkpoint 初始化策略。默认使用 balance 难度的 checkpoint。"
        ),
    )
    parser.add_argument(
        "--no-init-from",
...
def load_checkpoint_if_available(agent, checkpoint_path, device, obs_dim, act_dim):
    ...
    print(f"init_from: loaded policy from {checkpoint_path}")
...
def main():
    ...
    env = CartPoleRecoverEnv(render_mode=None)
