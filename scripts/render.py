...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Render a trained policy for any configured task.",
    )
    parser.add_argument(
        "--task",
        required=True,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="可选：指定 checkpoint；不填则使用该任务默认 checkpoint。",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    if args.random:
        ...
        if not model_path.exists():
            raise FileNotFoundError(
                ...
                f"Train it with: python scripts/train.py --task {args.task}, "
                "or add --random to inspect the environment without a model."
            )

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        agent, checkpoint = load_actor_critic(model_path, device=device)
