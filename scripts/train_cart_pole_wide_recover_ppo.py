...
DEFAULT_INIT_PATH = ROOT / "checkpoints" / "ppo_cart_pole_recover_torch.pt"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train CartPoleWideRecoverEnv with PPO.",
    )
    parser.add_argument(
        "--init-from",
        type=Path,
        ...
        help=(
            "用已有 checkpoint 初始化策略。默认使用 recover 难度的 checkpoint。"
        ),
    )
    parser.add_argument(
        "--no-init-from",
...
def main():
    ...
    env = CartPoleWideRecoverEnv(render_mode=None)
