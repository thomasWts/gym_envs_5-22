...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a MuJoCo Gymnasium task with the shared PyTorch PPO trainer.",
    )
    parser.add_argument(
        "--task",
        required=True,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="可选：从指定 checkpoint 初始化；不填则使用该任务默认的课程学习 checkpoint。",
    )
    parser.add_argument(
        "--no-init-from",
        action="store_true",
